<template>
  <div>
    ...
    <div v-for="(info, clave) in rutas" :key="clave">
      <button
        class="accordion"
        @click="togglePanel(clave)"
        :class="{ active: clave === rutaSeleccionada }"
      >
        {{ info.texto }}
        <span class="arrow" :class="{ rotated: clave === rutaSeleccionada }"
          >&#x2192;</span
        >
      </button>
...
export default {
  ...
  onMounted() {
    this.rutaSeleccionada = this.$route.params.ruta;
  },
};
</script>
